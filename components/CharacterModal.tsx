...
INSTRUCTION: Output ONLY the raw content for this field. Do not include explanations, markdown blocks (unless requested), or labels. Be creative and consistent.`;
  ...
  const renderFieldControls = (field: keyof Character, type: 'input' | 'textarea') => {
    ...
    // Enhanced positioning: Top-right for textareas to avoid covering text, centered-right for inputs
    const positionClass = isInput
      ? "top-1/2 -translate-y-1/2 right-2"
      : "top-2 right-2";
    const containerClass = `absolute ${positionClass} flex items-center gap-1.5 z-10`;

    return (
        <div className={containerClass}>
            {autoFillMenuField === field ? (
                 <div className="flex items-center gap-1 bg-zinc-900/95 border border-zinc-700 rounded p-1 shadow-xl animate-in fade-in zoom-in duration-200 backdrop-blur-md">
                ...
                <button
                    ...
                    className="p-1.5 bg-zinc-900/95 hover:bg-zinc-800 text-zinc-400 hover:text-orange-400 rounded-md transition-colors border border-zinc-700/50 backdrop-blur-md shadow-lg"
                    title="Auto Fill"
                >
            ...
            <button
                ...
                className="p-1.5 bg-zinc-900/95 hover:bg-zinc-800 text-zinc-400 hover:text-orange-500 rounded-md transition-colors border border-zinc-700/50 backdrop-blur-md disabled:opacity-30 disabled:cursor-not-allowed shadow-lg"
