                                                    ...
                                                    <div className="p-2 border-b border-zinc-800 sticky top-0 bg-zinc-900 z-10">
                                                        <div className="flex items-center gap-2">
                                                            <input
                                                                ...
                                                                className="flex-1 bg-black border border-zinc-700 px-2 py-1 text-xs text-zinc-300 placeholder-zinc-600 outline-none focus:border-orange-500/50"
                                                                autoFocus
                                                            />
                                                            <button
                    ...
                    <div className="flex-1 space-y-8">
                         {localSettings.apiProvider === 'puter' ? (
                            <section>
                                ...
                                <div className="bg-orange-950/10 border border-orange-900/30 p-6 rounded-lg text-center">
                                    <div className="flex justify-center mb-3">
                                        <div className="text-orange-500/50">
                                            <Sparkles size={32} />
                                        </div>
                                    </div>
                                    <p className="text-sm text-zinc-400 mb-2 font-medium">Puter handles generation settings automatically</p>
                                    <p className="text-[10px] text-zinc-600 leading-relaxed">
                                        When using Puter, all generation parameters are managed by the Puter AI service.
                                        You only need to select your preferred model above.
                                    </p>
                                </div>
                            </section>
                         ) : (
                         <section>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="p-3 bg-red-950/20 rounded border border-red-900/50">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2">
                                            <AlertTriangle size={14} className="text-red-500" />
                                            <label className="text-[10px] font-bold text-red-400 uppercase">Ultra Jailbreak Mode</label>
                                        </div>
                                        <button
                                            onClick={() => handleChange('jailbreakEnabled', !localSettings.jailbreakEnabled)}
                                            className={`transition-colors ${localSettings.jailbreakEnabled ? 'text-red-500' : 'text-zinc-700'}`}
                                        >
                                            {localSettings.jailbreakEnabled ? <ToggleRight size={20} /> : <ToggleLeft size={20} />}
                                        </button>
                                    </div>
                                    <p className="text-[9px] text-zinc-500 leading-relaxed">
                                        Activates maximum strength jailbreak for Puter. Disables ALL content filters and safety protocols. Models will generate ANY requested content without refusal or filtering. Use responsibly for mature fiction writing only.
                                    </p>
                                </div>
